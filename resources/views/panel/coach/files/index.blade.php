...
<x-panel-layout>
    <div class="bg-white overflow-hidden shadow-xl rounded-lg p-4 sm:p-8">
        <div class="flex justify-between items-center mb-4">
            <h1 class="my-heading text-2xl">{{ __('Club Files') }}</h1>
            <span class="text-sm text-gray-600">{{ $files->total() }} {{ __('files total') }}</span>
        </div>

        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
            <form id="filter-form" method="GET" action="{{ route('panel.coach.files.index') }}" class="flex flex-col sm:flex-row gap-4 flex-wrap">
                <div class="flex-1">
        ...
        <div id="results">
            @include('panel.coach.files._table', compact('files'))
        </div>
    </div>
</x-panel-layout>