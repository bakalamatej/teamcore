<div class="border border-gray-300 rounded-md overflow-hidden overflow-x-auto shadow-md">
	<table class="w-full data-table min-w-[600px]">
			...
			@forelse($events as $event)
				<tr class="border-b hover:bg-gray-50 data-row">
					...
					<td class="p-3 text-right">
						<a href="{{ route('panel.coach.events.show', $event) }}" class="table-action view mr-2">
							{{ __('View') }}
						</a>

						<a href="{{ route('panel.coach.events.edit', $event) }}" class="table-action edit mr-2">
							{{ __('Edit') }}
						</a>
						...
						<x-modal name="confirm-event-deletion-{{ $event->event_id }}" :show="false" focusable>
							<form method="POST" action="{{ route('panel.coach.events.destroy', $event) }}" class="p-6 text-left">
								@csrf
								@method('DELETE')
