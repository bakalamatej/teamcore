                ...
                <tr class="border-b hover:bg-gray-50 data-row">
                    ...
                    <td class="p-3 text-right">
                        ...
                        @if(Auth::user()->user_id === $file->uploaded_by_user_id)
                            @php
                                $memberClubId = $file->memberClubs->first()?->member_club_id;
                                $clubId = $file->clubs->first()?->club_id;
                                $eventId = $file->events->first()?->event_id;

                                if ($memberClubId) {
                                    $modelType = 'member_club';
                                    $modelId = $memberClubId;
                                } elseif ($clubId) {
                                    $modelType = 'club';
                                    $modelId = $clubId;
                                } elseif ($eventId) {
                            ...
                            @if($modelType && $modelId)
                                ...
                                <x-modal name="delete-file-{{ $file->file_id }}" :show="false" focusable>
                                    <form method="POST" action="{{ route('files.web.delete', [$modelType, $modelId, $file->file_id]) }}" class="p-6 text-left">
                                        @csrf
                                        @method('DELETE')
