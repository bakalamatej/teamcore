            ...
            @forelse($fieldTypes as $fieldType)
                <tr class="border-b hover:bg-gray-50 data-row">
                    ...
                    <td class="p-3 text-right">
                        <a href="{{ route('panel.field-types.edit', $fieldType) }}" class="table-action edit mr-2">
                            {{ __('Edit') }}
                        </a>
                        ...
                        <x-modal name="confirm-field-type-deletion-{{ $fieldType->field_type_id }}" :show="false" focusable>
                            <form method="POST" action="{{ route('panel.field-types.destroy', $fieldType) }}" class="p-6 text-left">
                                @csrf
                                @method('DELETE')
