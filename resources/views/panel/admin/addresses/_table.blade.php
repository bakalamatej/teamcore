            ...
            @forelse($addresses as $address)
                <tr class="border-b hover:bg-gray-50 data-row">
                    ...
                    <td class="p-3 text-right">
                        <a href="{{ route('panel.admin.addresses.edit', $address) }}" class="table-action edit mr-2">
                            {{ __('Edit') }}
                        </a>
                        ...
                        <x-modal name="confirm-address-deletion-{{ $address->address_id }}" :show="false" focusable>
                            <form method="POST" action="{{ route('panel.admin.addresses.destroy', $address) }}" class="p-6 text-left">
                                @csrf
                                @method('DELETE')
