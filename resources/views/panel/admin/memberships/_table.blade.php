            ...
            @forelse($memberships as $membership)
                <tr class="border-b hover:bg-gray-50 data-row">
                    ...
                    <td class="p-3 text-right">
                        <a href="{{ route('panel.admin.memberships.edit', $membership) }}" class="table-action edit">
                            {{ __('Edit') }}
                        </a>
                        ...
                        <x-modal name="confirm-membership-deletion-{{ $membership->member_club_id }}" :show="false" focusable>
                            <form method="POST" action="{{ route('panel.admin.memberships.destroy', $membership) }}" class="p-6 text-left">
                                @csrf
                                @method('DELETE')
