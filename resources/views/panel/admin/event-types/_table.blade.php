            ...
            @forelse($eventTypes as $eventType)
                <tr class="border-b hover:bg-gray-50 data-row">
                    ...
                    <td class="p-3 text-right">
                        <a href="{{ route('panel.admin.event-types.edit', $eventType) }}" class="table-action edit mr-2">
                            {{ __('Edit') }}
                        </a>
                        ...
                        <x-modal name="confirm-event-type-deletion-{{ $eventType->event_type_id }}" :show="false" focusable>
                            <form method="POST" action="{{ route('panel.admin.event-types.destroy', $eventType) }}" class="p-6 text-left">
                                @csrf
                                @method('DELETE')
