        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="space-y-2">
                            @foreach($tournament->clubs as $club)
                                <div class="detail-list-item">
                                    <a href="{{ route('panel.admin.clubs.show', $club) }}" class="detail-list-item-link">{{ $club->name }}</a>
                ...
                <div class="detail-card">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="flex gap-2">
                            <x-add-button href="{{ route('panel.admin.tournaments.children.create', $tournament) }}" class="!h-9" />
                        </div>
                    </div>
                                        ...
                                        @foreach($childEvents as $child)
                                            <tr class="border-b hover:bg-gray-50">
                                                ...
                                                <td class="p-3 text-right">
                                                    <a href="{{ route('panel.admin.events.show', $child) }}" class="table-action view mr-2">{{ __('View') }}</a>
                                                    <a href="{{ route('panel.admin.events.edit', $child) }}" class="table-action edit mr-2">{{ __('Edit') }}</a>
                                                    ...
                                                    <x-modal name="detach-child-{{ $child->event_id }}" :show="false" focusable>
                                                        <form method="POST" action="{{ route('panel.admin.tournaments.children.detach', [$tournament, $child]) }}" class="p-6 text-left">
                                                            @csrf
                                                            @method('DELETE')
                    ...
                    @if($availableEvents->count() > 0)
                        <div class="mt-4 pt-4 border-t border-gray-200">
                            ...
                            <form method="POST" action="{{ route('panel.admin.tournaments.children.attach', $tournament) }}" class="flex items-center gap-2 items-end">
                                @csrf
                                <div class="flex-1">
                                    <x-select-input
                                        id="event_id"
            ...
            <div class="lg:col-span-1 flex flex-col">
                ...
                <div class="space-y-3 mt-auto">
                    <x-primary-button class="w-full justify-center" :href="route('panel.admin.tournaments.edit', $tournament)">
                        {{ __('Edit Tournament') }}
                    </x-primary-button>
                    ...
                    <x-modal name="confirm-tournament-deletion-{{ $tournament->event_id }}" :show="false" focusable>
                        <form method="POST" action="{{ route('panel.admin.tournaments.destroy', $tournament) }}" class="p-6 text-left">
                            @csrf
                            @method('DELETE')
