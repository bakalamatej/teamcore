...
<x-panel-layout>
    <div class="bg-white overflow-hidden shadow-xl rounded-lg sm:p-8">
        ...
        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
            <form id="filter-form" method="GET" action="{{ route('panel.admin.tournaments.index') }}" class="flex flex-col sm:flex-row gap-4 flex-wrap">
                <div class="flex-1">
                ...
                <div class="flex items-end justify-end">
                    <x-add-button href="{{ route('panel.admin.tournaments.create') }}" class="!h-9" />
                </div>
            </form>
        </div>

        <div id="results">
            @include('panel.admin.tournaments._table', compact('tournaments'))
        </div>
    </div>
</x-panel-layout>