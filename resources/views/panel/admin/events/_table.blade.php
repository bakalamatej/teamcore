            ...
            @forelse($events as $event)
                <tr class="border-b hover:bg-gray-50 data-row">
                    ...
                    <td class="p-3 text-right">
                        <a href="{{ route('panel.admin.events.show', $event) }}" class="table-action view mr-2">
                            {{ __('View') }}
                        </a>

                        <a href="{{ route('panel.admin.events.edit', $event) }}" class="table-action edit mr-2">
                            {{ __('Edit') }}
                        </a>
                        ...
                        <x-modal name="confirm-event-deletion-{{ $event->event_id }}" :show="false" focusable>
                            <form method="POST" action="{{ route('panel.admin.events.destroy', $event) }}" class="p-6 text-left">
                                @csrf
                                @method('DELETE')
