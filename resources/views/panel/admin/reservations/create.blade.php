<x-panel-layout>
	<div class="bg-white overflow-hidden shadow-xl rounded-lg p-4 sm:p-8">
		...
		<form
			method="POST"
			action="{{ route('panel.admin.reservations.store') }}"
			class="space-y-6"
			x-data="reservationForm"
			x-init="
				selectedMembership = @js((string) old('created_by_member_club_id', ''));
				selectedSportField = @js((string) old('sport_field_id', ''));
				...
				<div class="flex-1 space-y-4">
					...
					<div>
						...
						<x-filtered-select
							...
							:placeholder="__('Select coach membership')"
						/>
					...
					<div>
						...
						<x-text-input
							...
							type="datetime-local"
							class="mt-1 block w-full"
							value="{{ old('end_date', now()->addHour()->format('Y-m-d\TH:i')) }}"
							required
						/>
