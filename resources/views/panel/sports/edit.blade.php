<x-app-layout>
    <div class="flex min-h-[calc(100vh-11rem)]">
        ...
        <form method="POST" action="{{ route('panel.sports.update', $sport) }}" class="space-y-4 max-w-2xl">
            @csrf
            @method('PATCH')
            ...
            <div class="flex gap-4 mt-6">
                ...
                <x-danger-button type="button" onclick="window.location='{{ route('panel.sports.index') }}'">
                    {{ __('Discard') }}
                </x-danger-button>
            </div>
        </form>
